...
// increase the number of calls using mutex to avoid race conditions.
func (m *mockPriceService) incNumCalls() {
	defer m.mu.Unlock()
	m.mu.Lock()
	m.numCalls++
}
...
// Check that parallelize service calls returning multiple errors
func TestGePricesFor_ParallelizeCallsWithErrors(t *testing.T) {
	errs := []error{
		errors.New("price service error #1"),
		errors.New("price service error #2"),
	}
	...
	mockService := &mockPriceService{
		...
		mockResults: map[string]mockResult{
			"p1": {price: 5, err: nil},
			"p2": {price: 7, err: errs[0]},
			"p3": {price: 9, err: errs[1]},
		},
	}
	cache := NewTransparentCache(mockService, time.Minute)
	prices, err := cache.GetPricesFor("p1", "p2", "p3")
	assertFloats(t, []float64{}, prices, "wrong prices returned")
	assertErrors(t, errs, err, "unexpected error")
}
